  ...
  for (int deviceId = 0; deviceId < numberOfGPUs; deviceId++)
  {
    ...
    for (int i = deviceId * rowsPerGPU; i < maxRow && i < rows; i++)
    {
      ...
      case 1:
        ...
        GPU::testRowL1<<<grid, block, sizeof(double) * numthreads, stream>>>(*state, curTask.row, curTask.rowCount);
        break;
      case 2:
        grid = dim3(curTask.rowCount * (curTask.row + curTask.rowCount), (int)state->p);
        GPU::testRowLN<4, 2><<<grid, block, 0, stream>>>(*state, curTask.row, curTask.rowCount);
        break;
      case 3:
        grid = dim3(curTask.rowCount * (curTask.row + curTask.rowCount), (int)state->p);
        GPU::testRowLN<5, 3><<<grid, block, 0, stream>>>(*state, curTask.row, curTask.rowCount);
        break;
      }
    }
  }
